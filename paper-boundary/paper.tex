...
\item \textbf{Onset-vs-harm is confounded with dose-driven degeneration.} Flipping the aligned
  ...
  resulting completions are often degenerate. So the HarmBench decay is consistent with
  ``large dose $\rightarrow$ incoherent output $\rightarrow$ no scoreable harm'' as well as with alignment
  decoupling onset from harm; a coherence- or perplexity-matched harm measurement is needed to
  separate them, and we do not yet localize \emph{why} onset-control outlives harm-control.
