...
harm at the crossing dose decays with alignment (\num{0.80} to \num{0.05} on OLMo, replicated on Qwen;
...
The invariance is not an artifact of how we fit the direction. A gradient-trained steering direction,
six times more efficient (mean \num{26.1} vs \num{4.48} over the four E1 checkpoints) and near-orthogonal to diff-in-means (cosine
\num{0.21}), is similarly near-constant (efficacy CV \num{0.196}, against \num{0.13} for difference-in-means). The invariance holds across mid-to-late
...
so the fixed-dose audit artifact appears for refusal but not honesty. Right: margin growth and the misranking replicate across Qwen3-8B, Llama-3.1-8B,
...
developmental reading of one failure of control: the per-dose steering lever is invariant across the
