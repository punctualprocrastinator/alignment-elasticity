...
per-dose steering efficacy stays nearly constant (coefficient of variation \num{0.13}, a \num{1.6}\x{} spread whose maximum is the aligned model itself; a
gradient-trained direction, six times more efficient and near-orthogonal, is similarly near-constant (efficacy CV \num{0.196}). A fixed-magnitude intervention therefore fails on aligned models for one banal reason:
...
A pre-registered non-inferiority test (equivalence margin $\Delta = 0.20\times$ base efficacy $= 0.77$;
...
CIs are in Table~\ref{tab:effci} (Appendix~A) and drawn on Figure~\ref{fig:lever}. Over the same span the margin climbs \num{9.5}\x, from \num{0.81} to
...
chiefly because the margin widened. Point estimates; per-checkpoint bootstrap CIs are in the released JSON.}
\label{fig:lever}
\end{figure}
...
a clean answer: the direction changed far less than the margin, and for refusal did not weaken at all.
