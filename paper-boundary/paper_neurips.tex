...
direction still flips an aligned model's refusal onset to compliance, but the peak HarmBench-judged
genuine harm achievable across the dose sweep decays with alignment (\num{0.80} to \num{0.05} on OLMo, replicated on Qwen;
this decay is confounded with dose-driven degeneration and is a hypothesis pending a coherence-matched
measurement, \S\ref{sec:limits}), and prefix-based
...
dose, where the onset flips, harm is \num{0}: the large dose merely degenerates the output). The same dissociation replicates on Qwen (genuine harm
...
\item \textbf{Onset-vs-harm is confounded with dose-driven degeneration.} Flipping the aligned
  model's onset requires a large dose \emph{because} its margin is wide, and Appendix~B shows the
  resulting completions are often degenerate. A cheap coherence proxy makes the confound concrete:
  the aligned Instruct model's onset-flipped completions are \SI{93}{\percent} degenerate (near-total
  token repetition) against \SI{17}{\percent} at base, so the HarmBench decay is plausibly
  ``large dose $\rightarrow$ incoherent output $\rightarrow$ no scoreable harm'' rather than alignment
  decoupling onset from harm. A coherence-gated harm re-measurement (released) is the definitive test;
  until it is run we present the second warning as a hypothesis, not a finding.
...
overstates compliance on exactly those aligned models: onset-control appears to outlive harm-control (a second warning confounded with dose-driven degeneration, \S\ref{sec:limits}), and
